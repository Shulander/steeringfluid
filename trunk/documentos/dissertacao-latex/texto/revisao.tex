...
SPH (smoothed particle hydrodynamics) é um método computacional usado para simular fluxo de fluidos. O mesmo é usado em vários campos de pesquisa, incluindo astrofísica, balística, vulcanologia e oceanologia. O mesmo é um método Baseado em partículas ~\cite{Monaghan1988}.

O método de SPH funciona dividindo o fluido em um conjunto de elementos discretos, denominados de partículas. Essas partículas possuem uma distância espacial (conhecida como {\it smoothing length}, tipicamente representado na equação por h), as quais suas propriedades são suavizadas por uma função de {\it kernel}. Isso significa que qualquer valor físico de qualquer partícula pode ser obtido somando as propriedades relevantes de todas as partículas as quais estão nas proximidades do {\it kernell}. 
